...
class MentorHomeController extends Controller
{
    ...
    {
        $images=MentorHome::orderByDesc('created_at')->get();
        $data=[
            'images'=>$images,
        ]; 
        return Inertia::render('Admin/MentorHome/Index',$data); 
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('image')->isValid()) {
            ...
            $resizedImageL = $imageL->resize(500,326)->encode(new AutoEncoder(quality: 90));
        ...
        return redirect()->route('homementor.index')->withSuccess($message);
    }
    ...
    {
        $title= $image =MentorHome::where('id',$id)->first();
