...
{
    /**
     * Display a listing of the resource. StudentHome homestudent
     */
    public function index()
    {
        $images= StudentHome::orderByDesc('created_at')->get();
        $data=[
            'images'=>$images,
        ];
        return Inertia::render('Admin/HomeStudent/Index',$data); 
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('image')->isValid()) {
            ...
            $resizedImageL = $imageL->resize(771,583)->encode(new AutoEncoder(quality: 90));
            $resizedImage = $image->resize(328,326)->encode(new AutoEncoder(quality: 20));
        ...
        return redirect()->route('homestudent.index')->withSuccess($message);
    }
    ...
    {
        $title= $image= StudentHome::where('id',$id)->first();
