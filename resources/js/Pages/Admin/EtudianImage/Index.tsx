...
const Index :React.FC<ProposImageType> = ({images}) =>{
    ...
    const { data, setData, post,reset, processing,progress, errors } = useForm({
        ...
        const submit:FormEventHandler=(e)=>{
            e.preventDefault();
            post(route('galeryetudian.store'), {
                preserveScroll: true,
                onSuccess: () => {
                    return reset() ;
                },
    ...
    <AdminLayout >
      <main className='w-full'>
        ...
        <Card className="w-[350px]">
            <CardHeader>
                <CardTitle>Image galery etudient</CardTitle>
            ...
            <CardContent>
                ...
                <div className="grid w-full items-center gap-4">
                    ...
                    <div className="flex flex-col space-y-1.5">
                    <Label htmlFor="name">Etudiant</Label>
                    <Input id="student" placeholder="nom de l'etudiant"
                    value={data.student}
                    onChange={e => setData('student', e.target.value)} />
                    {errors.student && <div className='text-red-700'>{errors.student}</div>}
                    </div>
                    <div className="flex flex-col space-y-1.5">
            ...
            <Table>
                ...
                <TableBody>
                    {images && images.map((image,index)=>(  
                        <CardImage image={image} key={index} link={"galeryetudian.destroy"} student={true}/>
                    ))}
                </TableBody>
            </Table>
        </div>
