...
const CourseCard:React.FC<ModulePropsType> = ({courses}) => {
  ...
  const submit:FormEventHandler=(e)=>{
    ...
    const formattedOptions = courses.map((option:PropsCourse) => ({
      value: option.id,
      label: option.title, 
    }));
    return (
    <Card className="w-[350px]">
        <CardHeader>
          <CardTitle>Creer un module</CardTitle>
          <CardDescription>Ajouter un module a un cours.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit}>
            <div className="grid w-full items-center gap-4 my-2">
                <div className="flex flex-col space-y-1.5">
                <Label htmlFor="title">Choisir un cours</Label>
                <Select
                ...
                />
                {errors.id && <div className='text-red-700'>{errors.id}</div>}
                </div>
                <div className="flex flex-col space-y-1.5 my-2">
                <Label htmlFor="modultitle">Titre</Label>
                <Input id="modultitle" placeholder="titre du module"
                 value={data.title}
